	...
	navigator.geolocation.getCurrentPosition(
		function (position) {
			...
			L.marker([lat, lng], {
				icon: L.divIcon({
					className: "custom-marker",
					html: '<div style="background:#2196f3;width:16px;height:16px;border-radius:50%;border:3px solid #fff;box-shadow:0 1px 6px rgba(0,0,0,0.5);"></div>',
					iconSize: [16, 16],
					iconAnchor: [8, 8],
				}),
			})
			...
			fetch(
				...
				.then(function (nearestShelters) {
					...
					if (nearestShelters.length === 0) {
						listEl.innerHTML =
							'<p class="empty-state">No shelters found nearby.</p>';
						return;
					}
		...
		function (error) {
			statusEl.textContent = "Unable to get location: " + error.message;
		}
	);
});
