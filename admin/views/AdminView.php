...
<body>
    ...
    <main class="admin-container">
        ...
        <div class="admin-board panel">
            <div class="admin-tabs" role="tablist" aria-label="Admin datasets">
                <a id="events-tab" class="admin-tab <?= $activeTab === 'events' ? 'active' : '' ?>"
                    href="<?= e(adminUrl(['tab' => 'events'])) ?>">Disaster Events</a>
                <a id="shelters-tab" class="admin-tab <?= $activeTab === 'shelters' ? 'active' : '' ?>"
                    href="<?= e(adminUrl(['tab' => 'shelters'])) ?>">Shelters</a>
                <a id="routes-tab" class="admin-tab <?= $activeTab === 'routes' ? 'active' : '' ?>"
                    href="<?= e(adminUrl(['tab' => 'routes'])) ?>">Evacuation Routes</a>
            </div>

            <div class="admin-toolbar">
                <form class="toolbar-group toolbar-search" method="get"
                    action="<?= e(adminUrl([$currentPageKey => 1])) ?>">
                    <input type="hidden" name="tab" value="<?= e($activeTab) ?>">
                    <input type="search" name="<?= e($currentSearchKey) ?>" value="<?= e($currentSearchValue) ?>"
                        placeholder="<?= e($currentSearchPlaceholder) ?>"
                        aria-label="<?= e($currentSearchPlaceholder) ?>">
                    <button type="submit">Search</button>
                </form>

                <form class="toolbar-group toolbar-size" method="get"
                    action="<?= e(adminUrl([$currentPageKey => 1])) ?>">
                    <input type="hidden" name="tab" value="<?= e($activeTab) ?>">
                    ...
                    <select id="pageSizeSelect" name="<?= e($currentSizeKey) ?>" onchange="this.form.submit()">
                        <?php foreach ($pageSizes as $size): ?>
                            <option value="<?= $size ?>" <?= (int) $currentPageSize === $size ? 'selected' : '' ?>><?= $size ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                </form>
                ...
                <div class="actions-dropdown">
                    ...
                    <div class="actions-dropdown-content" id="actionsMenuContent">
                        ...
                        <a href="/api/export?resource=<?= e($activeTab) ?>&format=csv"
                            class="dropdown-item btn-export-item" download>
                            <span class="icon-export">&#128196;</span> Export CSV
                        </a>
                        <a href="/api/export?resource=<?= e($activeTab) ?>&format=json"
                            class="dropdown-item btn-export-item" download>
            ...
            <div class="tab-panels">
                <section class="tab-panel <?= $activeTab === 'events' ? 'active' : '' ?>" id="events-panel"
                        ...
                        <table class="dashboard-table">
                            <thead>
                                <tr>
                                    ...
                                    <th><?= $sortLink('events', 'Description', 'description', $eventsSort, $eventsSortDir) ?>
                                    </th>
                                    <th><?= $sortLink('events', 'Severity', 'severity', $eventsSort, $eventsSortDir) ?>
                                    </th>
                                    <th><?= $sortLink('events', 'Status', 'status', $eventsSort, $eventsSortDir) ?></th>
                                    <th><?= $sortLink('events', 'Latitude', 'latitude', $eventsSort, $eventsSortDir) ?>
                                    </th>
                                    <th><?= $sortLink('events', 'Longitude', 'longitude', $eventsSort, $eventsSortDir) ?>
                                    </th>
                                    <th><?= $sortLink('events', 'Created', 'created', $eventsSort, $eventsSortDir) ?>
                                    </th>
                                    <th><?= $sortLink('events', 'Updated', 'updated', $eventsSort, $eventsSortDir) ?>
                                    </th>
                                    <th>Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php if (!$hasEvents): ?>
                                    <tr>
                                        <td colspan="10" class="empty-state-cell">No events recorded.</td>
                                    </tr>
                                <?php endif; ?>
                                ...
                                <?php foreach ($events as $e): ?>
                                    <tr data-id="<?= $e['id'] ?>">
                                        ...
                                        <td data-label="Title">
                                            ...
                                            <input type="text" class="mode-edit field-title" value="<?= $e['title'] ?>"
                                                style="display:none;">
                                        </td>
                                        <td data-label="Description">
                                            ...
                                            <?php else: ?>
                                                <div class="description-cell">
                                                    ...
                                                    <textarea class="mode-edit field-description"
                                                        style="display:none;"><?= htmlspecialchars($e['description'], ENT_QUOTES, 'UTF-8') ?></textarea>
                                                    <button type="button" class="view-more-btn mode-view" data-open-description
                                                        data-title="<?= $e['title'] ?>"
                                                        data-description="<?= $e['description'] ?>">View more</button>
                                                </div>
                                            <?php endif; ?>
                                        </td>
                                        ...
                                        <td data-label="Latitude">
                                            ...
                                            <input type="number" step="0.001" class="mode-edit field-latitude"
                                                value="<?= $e['latitude_val'] ?>" style="display:none;">
                                        </td>
                                        <td data-label="Longitude">
                                            <span class="mode-view"><?= $e['longitude_disp'] ?></span>
                                            <input type="number" step="0.001" class="mode-edit field-longitude"
                                                value="<?= $e['longitude_val'] ?>" style="display:none;">
                                        </td>
                                        <td data-label="Created">
                                            <span
                                                class="mode-view timestamp-cell"><?php $parts = preg_split('/\s+/', $e['created_at'], 2);
                                        ...
                                        <td data-label="Updated">
                                            <span
                                                class="mode-view timestamp-cell"><?php $parts = preg_split('/\s+/', $e['updated_at'], 2);
                                                echo htmlspecialchars($parts[0]);
                                                if (!empty($parts[1])) {
                                                    echo '<br>' . htmlspecialchars($parts[1]);
                                                } ?></span>
                                        ...
                                        <td data-label="Actions">
                                            <button class="btn-action-edit mode-view" type="button"
                                                onclick="toggleEditRow(this)">Edit</button>
                                            <button class="btn-action-delete mode-view" type="button"
                                                onclick="dispatchDelete('event', <?= $e['id'] ?>)">Delete</button>

                                            <button class="btn-action-save mode-edit" type="button" style="display:none;"
                                                onclick="dispatchUpdate('event', <?= $e['id'] ?>, this)">Save</button>
                                            <button class="btn-action-cancel mode-edit" type="button" style="display:none;"
                                                onclick="toggleEditRow(this)">Cancel</button>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
                    ...
                    <?php if ($eventTotalPages > 1): ?>
                        <div class="pagination-bar">
                            ...
                            <div class="pagination-links">
                                <?php if ($eventsPage > 1): ?>
                                    <a
                                        href="<?= e(adminUrl(['tab' => 'events', 'events_page' => $eventsPage - 1])) ?>">Previous</a>
                                <?php endif; ?>
                ...
                <section class="tab-panel <?= $activeTab === 'shelters' ? 'active' : '' ?>" id="shelters-panel"
                    role="tabpanel" aria-labelledby="shelters-tab">
                    <div class="panel-header-row">
                    ...
                    <div class="table-responsive">
                        <table class="dashboard-table">
                            <thead>
                                <tr>
                                    <th><?= $sortLink('shelters', 'Name', 'name', $sheltersSort, $sheltersSortDir) ?>
                                    </th>
                                    <th><?= $sortLink('shelters', 'Address', 'address', $sheltersSort, $sheltersSortDir) ?>
                                    </th>
                                    <th><?= $sortLink('shelters', 'Type', 'type', $sheltersSort, $sheltersSortDir) ?>
                                    </th>
                                    <th><?= $sortLink('shelters', 'Capacity', 'capacity', $sheltersSort, $sheltersSortDir) ?>
                                    </th>
                                    <th><?= $sortLink('shelters', 'Status', 'status', $sheltersSort, $sheltersSortDir) ?>
                                    </th>
                                    <th><?= $sortLink('shelters', 'Phone', 'phone', $sheltersSort, $sheltersSortDir) ?>
                                    </th>
                                    <th><?= $sortLink('shelters', 'Notes', 'notes', $sheltersSort, $sheltersSortDir) ?>
                                    </th>
                                    <th><?= $sortLink('shelters', 'Created', 'created', $sheltersSort, $sheltersSortDir) ?>
                                    </th>
                                    <th><?= $sortLink('shelters', 'Updated', 'updated', $sheltersSort, $sheltersSortDir) ?>
                                    </th>
                                    <th>Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php if (!$hasShelters): ?>
                                    <tr>
                                        <td colspan="10" class="empty-state-cell">No shelter locations setup.</td>
                                    </tr>
                                <?php endif; ?>
                                ...
                                <?php foreach ($shelters as $s): ?>
                                    <tr data-id="<?= $s['id'] ?>">
                                        <td data-label="Name">
                                            ...
                                            <input type="text" class="mode-edit field-name" value="<?= $s['name'] ?>"
                                                style="display:none;">
                                        </td>
                                        <td data-label="Address">
                                            <span class="mode-view"><?= $s['address'] ?></span>
                                            <input type="text" class="mode-edit field-address" value="<?= $s['address'] ?>"
                                                style="display:none;">
                                        </td>
                                        <td data-label="Type">
                                        ...
                                        <td data-label="Capacity">
                                            ...
                                            <input type="number" class="mode-edit field-capacity"
                                                value="<?= $s['capacity'] ?>" style="display:none;">
                                        ...
                                        <td data-label="Phone">
                                            ...
                                            <input type="text" class="mode-edit field-contact_phone"
                                                value="<?= $s['contact_phone'] ?>" style="display:none;">
                                        </td>
                                        <td data-label="Notes">
                                            <span class="mode-view note-cell"><?= $s['notes'] ?></span>
                                            <textarea class="mode-edit field-notes"
                                                style="display:none;"><?= $s['notes'] ?></textarea>
                                        </td>
                                        <td data-label="Created">
                                        ...
                                        <td data-label="Actions">
                                            <button class="btn-action-edit mode-view" type="button"
                                                onclick="toggleEditRow(this)">Edit</button>
                                            <button class="btn-action-delete mode-view" type="button"
                                                onclick="dispatchDelete('shelter', <?= $s['id'] ?>)">Delete</button>

                                            <button class="btn-action-save mode-edit" type="button" style="display:none;"
                                                onclick="dispatchUpdate('shelter', <?= $s['id'] ?>, this)">Save</button>
                                            <button class="btn-action-cancel mode-edit" type="button" style="display:none;"
                                                onclick="toggleEditRow(this)">Cancel</button>
                                        </td>
                                    </tr>
                        ...
                        <div class="pagination-bar">
                            ...
                            <div class="pagination-links">
                                <?php if ($sheltersPage > 1): ?>
                                    <a
                                        href="<?= e(adminUrl(['tab' => 'shelters', 'shelters_page' => $sheltersPage - 1])) ?>">Previous</a>
                                <?php endif; ?>
                                <?php if ($sheltersPage < $shelterTotalPages): ?>
                                    <a
                                        href="<?= e(adminUrl(['tab' => 'shelters', 'shelters_page' => $sheltersPage + 1])) ?>">Next</a>
                                <?php endif; ?>
                            </div>
                        </div>
                    <?php endif; ?>
                ...
                <section class="tab-panel <?= $activeTab === 'routes' ? 'active' : '' ?>" id="routes-panel"
                    role="tabpanel" aria-labelledby="routes-tab">
                    <div class="panel-header-row">
                        <h2>Historical Evacuation Routes</h2>
                        <p>Browse through evacuation routes, search by route or shelter, and sort by name, shelter,
                    ...
                    <div class="table-responsive">
                        <table class="dashboard-table">
                            <thead>
                                <tr>
                                    <th><?= $sortLink('routes', 'Route Name', 'name', $routesSort, $routesSortDir) ?>
                                    </th>
                                    <th><?= $sortLink('routes', 'Target Shelter', 'shelter', $routesSort, $routesSortDir) ?>
                                    </th>
                                    <th>Start Lat</th>
                                    <th>Start Lng</th>
                                    <th><?= $sortLink('routes', 'Distance (m)', 'distance', $routesSort, $routesSortDir) ?>
                                    </th>
                                    <th><?= $sortLink('routes', 'Duration (min)', 'duration', $routesSort, $routesSortDir) ?>
                                    </th>
                                    <th><?= $sortLink('routes', 'Status', 'status', $routesSort, $routesSortDir) ?></th>
                                    <th>Notes</th>
                                    <th><?= $sortLink('routes', 'Created', 'created', $routesSort, $routesSortDir) ?>
                                    </th>
                                    <th><?= $sortLink('routes', 'Updated', 'updated', $routesSort, $routesSortDir) ?>
                                    </th>
                                    <th>Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php if (!$hasRoutes): ?>
                                    <tr>
                                        <td colspan="11" class="empty-state-cell">No evacuation routes setup.</td>
                                    </tr>
                                <?php endif; ?>
                                ...
                                <?php foreach ($routes as $r): ?>
                                    <tr data-id="<?= $r['id'] ?>">
                                        <td data-label="Route Name">
                                            ...
                                            <input type="text" class="mode-edit field-name" value="<?= $r['name'] ?>"
                                                style="display:none;">
                                        </td>
                                        ...
                                        <td data-label="Start Lat">
                                            ...
                                            <input type="number" step="0.000001" class="mode-edit field-from_latitude"
                                                value="<?= $r['from_latitude'] ?>" style="display:none;">
                                        </td>
                                        <td data-label="Start Lng">
                                            <span class="mode-view"><?= $r['from_longitude'] ?></span>
                                            <input type="number" step="0.000001" class="mode-edit field-from_longitude"
                                                value="<?= $r['from_longitude'] ?>" style="display:none;">
                                        </td>
                                        ...
                                        <td data-label="Status">
                                            ...
                                            <select class="mode-edit field-status" style="display:none;">
                                                <option value="active" <?= $r['status'] === 'active' ? 'selected' : '' ?>>
                                                    Active</option>
                                                <option value="blocked" <?= $r['status'] === 'blocked' ? 'selected' : '' ?>>
                                                    Blocked</option>
                                                <option value="closed" <?= $r['status'] === 'closed' ? 'selected' : '' ?>>
                                                    Closed</option>
                                            </select>
                                        </td>
                                        <td data-label="Notes">
                                            ...
                                            <textarea class="mode-edit field-notes"
                                                style="display:none;"><?= $r['notes'] ?></textarea>
                                        </td>
                                        <td data-label="Created">
                                        ...
                                        <td data-label="Actions">
                                            <button class="btn-action-edit mode-view" type="button"
                                                onclick="toggleEditRow(this)">Edit</button>
                                            <button class="btn-action-delete mode-view" type="button"
                                                onclick="dispatchDelete('route', <?= $r['id'] ?>)">Delete</button>

                                            <button class="btn-action-save mode-edit" type="button" style="display:none;"
                                                onclick="dispatchUpdate('route', <?= $r['id'] ?>, this)">Save</button>
                                            <button class="btn-action-cancel mode-edit" type="button" style="display:none;"
                                                onclick="toggleEditRow(this)">Cancel</button>
                                        </td>
                                    </tr>
                    ...
                    <?php if ($routeTotalPages > 1): ?>
                        <div class="pagination-bar">
                            ...
                            <div class="pagination-links">
                                <?php if ($routesPage > 1): ?>
                                    <a
                                        href="<?= e(adminUrl(['tab' => 'routes', 'routes_page' => $routesPage - 1])) ?>">Previous</a>
                                <?php endif; ?>
    ...
    <div id="addModal" class="modal-overlay" hidden>
        <div class="modal-card" role="dialog" aria-modal="true" aria-labelledby="addModalTitle">
            ...
            <form class="modal-form <?= $activeTab === 'events' ? 'active' : '' ?>" data-entity-form="events"
                method="post" action="/admin/events">
                ...
                <div class="form-row">
                    ...
                    <select id="modal_event_severity" name="severity">
                        <?php foreach ($severityOptions as $opt): ?>
                            <option value="<?= $opt['value'] ?>" <?= $opt['value'] === 'moderate' ? 'selected' : '' ?>>
                                <?= $opt['label'] ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            ...
            <form class="modal-form <?= $activeTab === 'shelters' ? 'active' : '' ?>" data-entity-form="shelters"
                method="post" action="/admin/shelters">
                <input type="hidden" name="csrf_token" value="<?= $csrf ?>">
            ...
            <form class="modal-form <?= $activeTab === 'routes' ? 'active' : '' ?>" data-entity-form="routes"
                method="post" action="/admin/routes">
                <input type="hidden" name="csrf_token" value="<?= $csrf ?>">
    ...
    <div id="importModal" class="modal-overlay" hidden>
        <div class="modal-card modal-card--compact" role="dialog" aria-modal="true" aria-labelledby="importModalTitle">
            <button type="button" class="modal-close" data-close-import
                aria-label="Close import dialog">&times;</button>
            ...
            <form class="modal-form active" id="importForm" method="post" enctype="multipart/form-data"
                ...
                <div class="import-info-box">
                    ...
                    <?php if ($activeTab === 'events'): ?>
                        <code>title</code> (req), <code>description</code>, <code>event_type</code>
                        (earthquake/flood/fire/storm/other), <code>severity</code> (low/moderate/high/extreme),
                        <code>latitude</code>, <code>longitude</code>, <code>status</code> (active/resolved)
                    <?php else: ?>
                        <code>name</code> (req), <code>address</code> (req), <code>latitude</code> (req),
                        <code>longitude</code> (req), <code>capacity</code>, <code>current_occupancy</code>,
                        <code>shelter_type</code> (community/school/stadium/military/other), <code>status</code>
                        (open/full/closed), <code>contact_phone</code>, <code>notes</code>
                    <?php endif; ?>
                </div>
    ...
    <div id="descriptionModal" class="modal-overlay" hidden>
        <div class="modal-card modal-card--compact" role="dialog" aria-modal="true"
            ...
            <button type="button" class="modal-close" data-close-description
                aria-label="Close description dialog">&times;</button>
